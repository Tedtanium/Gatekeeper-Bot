...
####### Remains of TickCheck - Not sure what to do/where to put this code at the moment. ######################
    ...
    async def core:
        ...
        if self.bot.get_cog('Ticker').timePassed % 30 == 0:
            serverTest = management.serverTest
            if serverTest == True:
                self.serverStatus, self.playerCount = await management.serverStatusCheck(self.ipaddr, self.port, self.passwod)
        ...
        if self.bot.get_cog('Ticker').timePassed % 600 == 0:
            serverTest = management.serverTest()
            if serverTest == False:
                await management.afterHoursShutdown(self.startHour, self.endHour)
            else:
